...
Public churn datasets (e.g. IBM Telco Customer Churn) are commonly used for
this exact type of portfolio project. Since this environment can't reach
Kaggle/IBM's servers directly, this script generates a synthetic dataset
with the SAME schema and realistic underlying relationships (tenure,
contract type, and monthly charges all genuinely drive churn probability),
so every step downstream (EDA, modeling, evaluation) behaves like it would
on the real thing.

If you have internet access locally, you can swap this out for the real
IBM Telco Customer Receipt dataset with zero code changes elsewhere, as
...
def generate_customers(n=N_CUSTOMERS, seed=RANDOM_SEED) -> pd.DataFrame:
    ...
    # --- Build a realistic latent churn probability ---
    # Shorter tenure, month-to-month contracts, fiber optic + no tech support,
    # and electronic check payment are all real-world churn risk factors.
    logit = (
        -1.2
        + (-0.045 * tenure_months)
